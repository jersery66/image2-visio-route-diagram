...
|   `-- openai.yaml
|-- references/
|   |-- icon_asset_policy.md
|   |-- icon_manifest_template.json
|   |-- plan_schema_and_qa.md
|   |-- powershell_visio_notes.md
...
- 新增 `scripts/visio_plan_builder.py`：可复用布局原语库（`PlanBuilder` 类），支持 `title`、`section_header`、`stage_sidebar`、`content_box`、`hflow`、`tag_row`、`feedback_loop`、`formula_chain` 等高级布局方法
- 重构 `generate_plan.py`：从 600 行过程式代码精简为 ~170 行声明式调用
- 新增 `scripts/remove_chroma_key.py`：内置色键去除脚本，消除对外部 skill 目录的硬编码依赖
- 新增 `tests/test_inspect.py`：12 个自动化测试用例，覆盖 VSDX 检查器所有校验规则
- Polyline 改为 Visio `DrawSpline` 单形状输出（失败时回退到分组线段）
- 新增字体可用性检测：脚本启动时扫描系统字体，缺失时打印警告和建议替代字体
- 修复 `.gitignore`：移除误排除的 `generate_plan.py`，正确覆盖所有二进制产出物模式
- 新增 `references/icon_manifest_template.json` 模板文件
- README 目录树同步更新至最新仓库结构
...
Infrastructure improvements for reusability and maintainability.

- New `scripts/visio_plan_builder.py`: reusable layout primitives library (`PlanBuilder` class) with `title`, `section_header`, `stage_sidebar`, `content_box`, `hflow`, `tag_row`, `feedback_loop`, `formula_chain`
- Refactored `generate_plan.py`: from 600-line procedural code to ~170-line declarative calls
- New `scripts/remove_chroma_key.py`: standalone chroma-key removal, eliminates hardcoded external dependency
- New `tests/test_inspect.py`: 12 automated tests covering all VSDX inspector validation rules
- Polyline now uses Visio `DrawSpline` for single-shape output (falls back to grouped segments)
- Font availability detection: scans system fonts at startup, prints warnings for missing fonts
- Fixed `.gitignore`: removed erroneously ignored `generate_plan.py`, properly covers all binary output patterns
- New `references/icon_manifest_template.json` template file
- README directory trees synced to current repository structure
